...
public class Main {
	...
	public static void main(String[] args) {
		...
		do{//filtrar el numero para que sea valido
			comodin=0;
			System.out.println("Escribe un número entero mayor que 0");
			...
			if (comodin==0) {//Si todos los valores están permitidos, cambiamos la variable a true
				permitido=true;
			}else{
				System.out.println("Valor no permitido");
			}
		}while (permitido==false);
			for(int i=0; i<cadena.length(); i++){//Escribimos la primera fila de cada número
			...
			for(int i=0; i<cadena.length(); i++){//Escribimos la segunda fila de cada número
			...
			for(int i=0; i<cadena.length(); i++){//Escribimos la tercera fila de cada número
